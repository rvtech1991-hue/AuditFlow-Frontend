...
Last synthesized: 2026-08-07, spot-updated 2026-08-11 after a live multi-account QA/testing pass (session-storage auth isolation, tenant-scoping consistency fixes, Platform admin Audit log page + Auditor account status controls, executive dashboard custom date range — see inline dated notes throughout for specifics), spot-updated again 2026-08-22 after a QA pass surfaced three bugs: incorrect Tasks-grid pagination on the Overdue/At-risk filters, a generic FluentValidation error string instead of the real per-field message, and misaligned "Assigned to"/"Status" grid columns — plus adding a top-level React error boundary that didn't previously exist. See inline dated notes throughout for specifics. Against frontend `main` and backend commit `72e2130` (per the integration guide) as a baseline. Re-verify anything load-bearing against current code before relying on it long-term — this is a snapshot, not a live source.
...
- `5 - SQL_INVESTIGATION_QUERIES.md` — screen-by-screen and general-purpose read-only SQL queries for validating DB state while investigating a bug (enum decode reference included). Not a schema doc — see `DatabaseStructure.md` for that.
...
- **`AuditFlow.API`** — thin controllers (build Command/Query → `_mediator.Send` → wrap `ApiResponse<T>`), `GlobalApiExceptionHandler` (an `IExceptionHandler`, registered via `AddExceptionHandler<T>()` + `app.UseExceptionHandler()` — not a classic custom-middleware class despite the similar-sounding name used in older notes), `NotificationHub` (SignalR).
...
**Fixed 2026-08-22 — FluentValidation errors now carry the real per-field message.** `GlobalApiExceptionHandler` previously mapped every `FluentValidation.ValidationException` to a hardcoded `detail: "One or more validation errors occurred"` with no field-level `errors`, unlike automatic ASP.NET model-validation 400s (which always got the proper `ValidationProblemDetails` shape). A too-short login password, for example, surfaced that generic sentence instead of "Password must be at least 8 characters". `WriteValidationProblemAsync` now builds a real `ValidationProblemDetails` from the validator's `Errors` collection for both paths — same `errorCode: "VALIDATION_ERROR"`, but `detail` is now the first concrete field message. See `Documents/4 - BACKEND_INTEGRATION_GUIDE.md` §4/§14 for the full write-up.
...
| `tasks.ts` | TasksController | Largest file (~740 lines): CRUD, assign, status/reopen, comments, attachments (`POST /files/upload`, deliberately not the separate presigned-attachment flow — mixing the two caused duplicate rows, per an inline post-mortem), bulk validate/import, CSV/Excel template handling via SheetJS. `TASK_PAGE_SIZE = 10`, explicitly chosen for scale. **Fixed 2026-08-22:** the `"overdue"`/`"atrisk"` status filters used to narrow just the current server page down client-side, so the pagination control (built from the unfiltered total) disagreed with what was actually shown — `getTasks()` now loops `/tasks` at a larger page size to pull every row matching the other filters, filters *that* full set by the derived status, and paginates the filtered result itself (`fetchAllTasksForDerivedStatus`/`matchesDerivedStatus`/`paginate` in `tasks.ts`). Normal status filters are unaffected — still single-page server pagination as before. |
...
Reusable primitives in `src/components/ui/` (barrel-exported): `Badge`, `Button`, `Card`, `Chip`, `DonutChart`/`TrendChart` (inline SVG, no chart library), `FormField`/`FieldRow`, `Modal`, `Pagination`, `RowActionMenu`, `Table`+`CellPerson`, `Toast`, `Toggle`, `Tooltip`. `Table`'s `Column.align` prop exists specifically so a page can force left/center/right on a per-column basis, immune to the shared `.grid-table` stylesheet's positional `:has()` rules (which assume "last column = status badge, should be centered" — true for the Users table it was written for, false for e.g. the tenant accounts table, where that assumption previously mismatched the header/data alignment) — prefer `Table` over a hand-rolled `<table>` for this reason alone. **`TaskGridPage`'s "Assigned to"/"Status" columns hit the same mismatch (fixed 2026-08-22):** the same two generic rules — `th:has(+ th:last-child)` centering whichever header sits second-to-last, and `td:has(> .badge)` centering any cell with a badge — centered the "Assigned to" header but not its `CellPerson` values, and centered the Status *values* but not its header, so both columns' headers and data visibly disagreed. Both columns now pass `align: "left"` explicitly. If a new page adds a `Table` whose last two columns aren't "assignee, then a status badge," check this same interaction before assuming default alignment is correct. `src/lib/useClickOutside.ts` (added 2026-08-11) closes a menu/dropdown on an outside click; used by the avatar menu, role switcher, and `RowActionMenu` — apply it to any new dropdown-style component rather than leaving it open-until-retoggled.

**`ErrorBoundary` (added 2026-08-22, `src/components/ErrorBoundary.tsx`):** a class-component React error boundary wrapping the entire app in `main.tsx` (outside `QueryClientProvider`/`RoleProvider`/`BrowserRouter`), so a render-time exception anywhere shows a friendly "Something went wrong" card with a reload button instead of a blank white page — there was no error boundary anywhere in the app before this. It only catches render/lifecycle errors, not data-fetch errors (those stay TanStack Query's per-page `isError` state, unchanged).
